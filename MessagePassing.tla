...
(*       actor. So a datanode may be delivered an AddEntryRequest from the   *)
(*       writer first, then a Read request from the 2nd client after that, *)
(*       but the datanode process the read request first, and the add second.*)
(***************************************************************************)

ReadTimeoutForBookie(msgs, cid, datanode) ==
    \E msg \in DOMAIN msgs :
        /\ msgs[msg] = -1
        /\ msg.datanode = datanode
        /\ msg.cid = cid
        /\ msg.type \in {ReadRequestMessage, ReadResponseMessage}

WriteTimeoutForBookie(msgs, cid, datanode, recovery) ==
    \E msg \in DOMAIN msgs :
        /\ msgs[msg] = -1
        /\ msg.datanode = datanode
        /\ msg.cid = cid
...
IgnoreFurtherReadResponses(msg, ensemble) ==
    messages' = [m \in DOMAIN messages |-> IF msg = m
                                           THEN 0
                                           ELSE IF /\ m.datanode \in ensemble
                                                   /\ m.cid = msg.cid
